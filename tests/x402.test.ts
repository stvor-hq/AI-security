...
describe('x402 payment protocol', () => {
  ...
  it('generateMockPaymentHeader produces valid base64 JSON with real ECDSA signature', () => {
    const header = generateMockPaymentHeader('unused', MOCK_TO, MOCK_ASSET, AMOUNT_WEI);
    const decoded = JSON.parse(Buffer.from(header, 'base64').toString('utf8'));
    expect(decoded.version).toBe('x402/1');
    expect(decoded.payload.from).toMatch(/^[A-Za-z0-9+/=_-]+$/);
    expect(decoded.payload.amount).toBe(AMOUNT_WEI);
    expect(decoded.payload.signature).toMatch(/^[A-Za-z0-9+/=_-]+$/);
  ...
  it('verifyPaymentHeader rejects missing signature', () => {
    const raw = {
      ...
      network: 'sepolia',
      payload: {
        from: 'abc123', to: MOCK_TO, asset: MOCK_ASSET,
  ...
  it('x402Middleware accepts valid ECDSA payment header', () => {
    const middleware = x402Middleware(AMOUNT_WEI, 'Test');
    const header = generateMockPaymentHeader('unused', MOCK_TO, MOCK_ASSET, AMOUNT_WEI);
    const req = new Request('http://localhost/api/test', {
